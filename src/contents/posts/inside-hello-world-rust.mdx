...
書き込みには`LineWriter`構造体が使われています。私なりのコメントをコード内に追加してあります。
...
impl<'a, W: ?Sized + Write> Write for LineWriterShim<'a, W> {
    ...
    fn write(&mut self, buf: &[u8]) -> io::Result<usize> {
        ...
        // ただし、内部ライターWはフラッシュしない
        // 改行かわからないため
        // このメソッドはBufWriterのみフラッシュし、内部ライターWはフラッシュしない
        //
        // `write_all` は「必要なら複数回書くことが許される」ため、よりシンプル
...
`write_fmt`の実装はありません。この場合は`Write`トレイトのデフォルト実装が使われます。このトレイトの`write_fmt`メソッドは、フォーマットがなく、完全に決まっている文字列に対しては`write_all`メソッドを呼び出すようになっています。よって、上に示す`write_all`が呼び出されます。
